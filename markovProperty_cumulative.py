...
def read_df():
    ch_orders=pd.read_csv(r'D:\Concordia\Master_Of_Science\Dataset_aedo_june_2022\Text_Mining\allprojects\ph2_ChangeTime_Step1_DataPrep_8steps.csv')
...
def transition_matrixes(ch_orders):
    ...
    target_atr_list=[atr for atr in ch_orders.columns if "ChMagnitude_" in atr]
    states=count_initial.index
    N_list=[]
    P_list=[]
    #creating 1 step transition matrixes
    for i in range(1,len(target_atr_list)):
...
ch_orders,ch_orders_filtered,count_1,N_list,P_list,expected_p15,E15,U,uSum,p_value=run(6,3)
