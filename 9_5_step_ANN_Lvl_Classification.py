...
ch_orders,x_train_str,x_test_str,y_train,y_test=run_the_code("Lvl","Prime","Construction")
class_counts = np.bincount(y_train)
class_weights = dict(zip(range(len(class_counts)), 1 / class_counts))
ann=tf.keras.models.Sequential()
...
ann.fit(x_train_str,y_train_dummy,batch_size=32,epochs=500)
y_pred_test = np.argmax(ann.predict(x_test_str), axis=-1)
y_pred_train = np.argmax(ann.predict(x_train_str), axis=-1)
