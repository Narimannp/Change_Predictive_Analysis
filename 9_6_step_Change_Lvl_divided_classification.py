...
def divide_classification(ch_orders):
    ...
    return(ch_orders)
# ch_or=divide_classification(ch)
# a=ch_or.groupby(["Classification_1"]).count()
# b=ch_or.groupby(["Classification_2"]).count()
def ch_freq(ch_orders):
    dict_1_p=ch_orders.groupby("ProjectClassification")["PrimeChFreq_p"].mean()
    dict_1_n=ch_orders.groupby("ProjectClassification")["PrimeChFreq_n"].mean()
    ...
    ch_orders["Freq_Class_1_p"]=ch_orders["ProjectClassification"].map(dict_1_p)
    ch_orders["Freq_Class_1_n"]=ch_orders["ProjectClassification"].map(dict_1_n)
...
def svm_classification_w_gridsearch(x_train_str,x_test_str,y_train,y_test,kernel_input):
    ...
    params={"C":(0.001,0.1,0.01),"degree":(2,1,3),"gamma":(0.01,0.1,0.001)}
