...
def time_grouping(row):
    if row["DateCreated"]<row["20%"]:
        return("1")
    elif row["DateCreated"]<row["40%"]:
        return("2")
    elif row["DateCreated"]<row["60%"]:
        return("3")
    elif row["DateCreated"]<row["80%"]:
        return("4")
    else:
        return("5")
...
def data_prep(projects,ch_orders,ch_imputed):
    ...
    projects["20%"]= (projects['Start'] + pd.to_timedelta(projects['DurationNew']/5, unit='D'))
    projects["40%"]= (projects['Start'] + pd.to_timedelta(projects['DurationNew']*2/5, unit='D'))
    projects["60%"]= (projects['Start'] + pd.to_timedelta(projects['DurationNew']*3/5, unit='D'))
    projects["80%"]= (projects['Start'] + pd.to_timedelta(projects['DurationNew']*4/5, unit='D'))
    ...
    projects=projects[["ContractValue","DurationNew","IndustryGroup","ProjectType","OperatingUnit","Province","City","Start","20%","40%","60%","80%","End"]]
...
def run():
    ...
    merged.to_csv(r'D:\Concordia\Master_Of_Science\Dataset_aedo_june_2022\Text_Mining\allprojects\ph2_ChangeTime_Step1_DataPrep.csv',index=True)
